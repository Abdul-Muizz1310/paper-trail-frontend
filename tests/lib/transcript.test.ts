...
import { isTerminalErrorReason, DebateSchema, StateEventSchema } from "@/lib/schemas";
...
describe("extractJudgeReasoning", () => {
  ...
  it("P3 extracts ## Reasoning section", () => {
    ...
    expect(extractJudgeReasoning(md)).toBe(
      "The evidence strongly supports the claim.",
    );
  });
  ...
  it("P4 extracts ## Judgment section (alternative heading)", () => {
    const md = [
      "## Judgment",
      "The proponent had stronger citations.",
    ].join("\n");
    expect(extractJudgeReasoning(md)).toBe(
      "The proponent had stronger citations.",
    );
  ...
  it("P5 extracts ## Decision section", () => {
    const md = [
      "## Decision",
      "Based on the debate, this is FALSE.",
    ].join("\n");
    expect(extractJudgeReasoning(md)).toBe(
      "Based on the debate, this is FALSE.",
    );
  ...
  it("P7 falls back to Verdict section when no Reasoning exists", () => {
    ...
    const md = [
      "## Verdict",
      "The evidence strongly favors TRUE.",
    ].join("\n");
    expect(extractJudgeReasoning(md)).toBe(
      "The evidence strongly favors TRUE.",
    );
